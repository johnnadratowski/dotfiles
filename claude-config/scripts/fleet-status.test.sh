...
run(){ HOME="$FAKEHOME" COLUMNS="${COLUMNS:-160}" bash "$SCRIPT" --lanes-dir "$LANES" "$@" </dev/null 2>&1; }
row(){ run | grep -E "^ +. +$1 " ; }
...
# ── the header tally ─────────────────────────────────────────────────────────────────────
has "header counts live lanes"       "3 live"            "$(run | head -1)"
